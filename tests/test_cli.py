...
def test_start_success(sample_cfg: AppConfig) -> None:
    with (
        ...
        patch("ssh_socks_cli.cli.tunnel.start", return_value=12345),
    ):
...
def test_stop_running() -> None:
    with patch("ssh_socks_cli.cli.tunnel.stop", return_value=True):
...
def test_stop_not_running() -> None:
    with patch("ssh_socks_cli.cli.tunnel.stop", return_value=False):
        result = runner.invoke(app, ["stop"])
    assert result.exit_code == 0
...
def test_restart_success(sample_cfg: AppConfig) -> None:
    with (
        patch("ssh_socks_cli.cli.tunnel.stop"),
        patch("ssh_socks_cli.cli.config.load", return_value=sample_cfg),
        patch("ssh_socks_cli.cli.tunnel.start", return_value=99999),
    ):
...
def test_doctor_all_ok(sample_cfg: AppConfig) -> None:
    checks = [
        ...
        Check("corporate VPN", True, "none detected"),
    ]
    with (
        patch("ssh_socks_cli.cli.config.load", return_value=sample_cfg),
