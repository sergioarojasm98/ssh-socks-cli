...
def _write_profiles_ini(root: Path, content: str) -> None:
    """Helper to create a profiles.ini and the profile directories it references."""
    (root / "profiles.ini").write_text(content, encoding="utf-8")


def test_install_section_overrides_legacy_default(tmp_path: Path) -> None:
    """Firefox 67+: [Install...] Default= is authoritative over [Profile] Default=1."""
    legacy_profile = tmp_path / "Profiles" / "aaa.default"
    active_profile = tmp_path / "Profiles" / "bbb.actual"
    ...
    _write_profiles_ini(
        ...
        "Name=Personal\n"
        "IsRelative=1\n"
        "Path=Profiles/bbb.actual\n"
        "\n",
    )

    with patch("ssh_socks_cli.firefox._candidate_profile_roots", return_value=[tmp_path]):
        profiles = firefox.list_profiles()

    assert len(profiles) == 2
    legacy = next(p for p in profiles if p.name == "default")
    personal = next(p for p in profiles if p.name == "Personal")
    # The Install section points to bbb.actual → Personal is the real default
    assert personal.is_default is True
    assert legacy.is_default is False


def test_legacy_default_used_when_no_install_section(tmp_path: Path) -> None:
    """Firefox <67 fallback: [Profile] Default=1 is used when no [Install...] exists."""
    profile_dir = tmp_path / "Profiles" / "old.default"
...
def test_default_profile_returns_install_section_match(tmp_path: Path) -> None:
    """default_profile() must return the Install-designated profile, not the legacy one."""
    legacy = tmp_path / "Profiles" / "legacy.default"
    active = tmp_path / "Profiles" / "active.profile"
    ...
    _write_profiles_ini(
        tmp_path,
        "[InstallAABBCCDD]\n"
        "Default=Profiles/active.profile\n"
        "\n"
        "[Profile0]\n"
        "Name=Legacy\n"
        "IsRelative=1\n"
        ...
        "Name=Active\n"
        "IsRelative=1\n"
        "Path=Profiles/active.profile\n",
    )

    with patch("ssh_socks_cli.firefox._candidate_profile_roots", return_value=[tmp_path]):
        result = firefox.default_profile()
