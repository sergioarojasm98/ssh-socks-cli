...
Users with **always-on corporate VPNs** (Palo Alto GlobalProtect, Cisco AnyConnect, Zscaler, etc.) on their work laptops cannot easily route specific browser traffic *outside* the VPN tunnel. This creates friction when users want to:

- Keep personal browsing (webmail, banking, social media) off corporate-monitored networks
- Access geo-restricted content blocked by corporate egress policies
- Test services as an external user from the same machine
- Separate work and personal contexts on a single device

The typical workaround — running `ssh -D 1080 user@server` in a terminal and manually editing Firefox's proxy settings — is fragile, OS-specific, and forgotten on reboot.
...
- Developers, SREs, and security engineers with a corporate laptop forced onto an always-on VPN.
- Users who already own or can spin up a personal VPS / home server reachable via SSH.
...
> As a user, I want `ssh-socks doctor` to check that `ssh`/`autossh` exist, my key has the right permissions, the SSH host is reachable, and (optionally) whether a corporate VPN is active.
...
| F10 | `doctor` checks: binaries, key file perms, host reachability |
...
- Multi-tunnel / per-site routing (may come in v0.2)
- VPN detection beyond a best-effort check
...
- `ssh-socks firefox apply` produces a Firefox that does NOT leak DNS through the corporate VPN.
